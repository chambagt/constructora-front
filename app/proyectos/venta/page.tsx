...
export default function ProyectosVentaPage() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      // Filtrar solo proyectos de tipo venta
      const proyectosVenta = proyectosGuardados.filter((p: Proyecto) => p.tipo === "venta")
      setProyectos(proyectosVenta)
    }
  }, [showForm])
  ...
  return (
    <div className="container mx-auto py-10">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Proyectos de Venta</h1>
        <Button onClick={() => setShowForm(!showForm)}>
          {showForm ? "Cerrar Formulario" : "Nuevo Proyecto de Venta"}
      ...
      {showForm && (
        <div className="mb-8">
          <NuevoProyectoForm tipo="venta" />
        </div>
      )}

      <div className="mb-8">
        <h2 className="text-xl font-semibold mb-4">Proyectos de Venta Existentes</h2>
        ...
        {proyectos.length > 0 ? (
          ...
          <div className="bg-white dark:bg-zinc-800 rounded-lg p-8 text-center">
            <p className="text-gray-500 dark:text-gray-400">
              No hay proyectos de venta. Presiona el botón "Nuevo Proyecto de Venta" para crear uno.
            </p>
          </div>
        )}
      </div>
